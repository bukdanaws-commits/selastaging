...
export function useApproveWithdrawal() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ withdrawalId, method }: { withdrawalId: string; method: 'AUTO_DOKU' | 'MANUAL' }) =>
      adminApi.approveWithdrawal(withdrawalId, method),
    onSuccess: () => {
...
export function useCheckDokuStatus() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (withdrawalId: string) =>
      adminApi.checkDokuStatus(withdrawalId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'withdrawals'] })
    },
  })
