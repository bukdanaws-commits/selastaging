...
export function generateAllMockData(): MockDataBundle {
  ...
  const withdrawalStatuses: WithdrawalStatus[] = ['pending', 'pending', 'approved', 'processing', 'transferred', 'completed', 'completed', 'rejected', 'cancelled']
  const withdrawals: IWithdrawalRequest[] = withdrawalStatuses.map((status, idx) => {
    ...
    // Hybrid method: alternate between AUTO_DOKU and MANUAL
    const method = idx % 3 === 0 ? 'AUTO_DOKU' as const : 'MANUAL' as const
    ...
    if (status === 'approved' || status === 'processing' || status === 'transferred' || status === 'completed') {
      wd.approvedAt = randomDate(requestedAt, now)
      wd.approvedBy = 'user-super-admin-001'
    }
    if (status === 'processing') {
      if (method === 'AUTO_DOKU') {
        wd.dokuDisbursementId = `DOKU-DISB-${Date.now()}-${idx}`
        wd.dokuReferenceNo = `DOKU-REF-${Math.random().toString(36).slice(2, 10).toUpperCase()}`
        wd.dokuStatus = 'PROCESSING'
      }
    }
    if (status === 'transferred' || status === 'completed') {
      ...
      if (method === 'AUTO_DOKU') {
        wd.dokuDisbursementId = `DOKU-DISB-${Date.now()}-${idx}`
        wd.dokuReferenceNo = `DOKU-REF-${Math.random().toString(36).slice(2, 10).toUpperCase()}`
        wd.dokuStatus = 'COMPLETED'
        wd.transferNote = 'Auto-disbursed via DOKU Disbursement API'
        wd.transferProof = 'auto_doku'
      } else {
        wd.transferProof = 'https://example.com/bukti-tf-mock.jpg'
        wd.transferNote = 'Transfer manual via BCA mobile'
      }
    }
    if (status === 'completed') {
