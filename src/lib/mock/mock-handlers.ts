...
export async function handleMockRequest<T = unknown>(request: MockRequest): Promise<T> {
  ...
  // GET /api/v1/organizer/finance
  if (endpoint === '/api/v1/organizer/finance' && method === 'GET') {
    const user = getCurrentUser()
    const store = useMockStore.getState()
  ...
  // POST /api/v1/organizer/withdraw
  if (endpoint === '/api/v1/organizer/withdraw' && method === 'POST') {
    await mockDelay()
    const user = getCurrentUser()
    const store = useMockStore.getState()
    const b = body as Record<string, unknown>
  ...
  if (method === 'PATCH') {
    ...
    if (wdApproveMatch) {
      ...
      const user = getCurrentUser()
      const b = body as Record<string, unknown>
      const method = (b.method as 'AUTO_DOKU' | 'MANUAL') || 'MANUAL'
      useMockStore.getState().approveWithdrawal(wdApproveMatch.id, user?.id || '', method)
      return undefined as T
    }
  }
  ...
  // POST /api/v1/admin/withdrawals/:id/check-doku
  if (method === 'POST') {
    const wdCheckDokuMatch = matchRoute('/api/v1/admin/withdrawals/:id/check-doku', endpoint)
    if (wdCheckDokuMatch) {
      await mockDelay()
      useMockStore.getState().checkDokuStatus(wdCheckDokuMatch.id)
      const wd = useMockStore.getState().withdrawals.find((w: IWithdrawalRequest) => w.id === wdCheckDokuMatch.id)
      return { success: true, data: { status: wd?.status || 'processing', dokuStatus: wd?.dokuStatus || 'PROCESSING' } } as T
    }
  }
  ...
  // GET /api/v1/admin/verifications — returns empty list (feature removed)
  if (endpoint === '/api/v1/admin/verifications' && method === 'GET') {
    const { page, perPage } = extractPagination(params)
    return paginate([] as unknown[], page, perPage) as T
  }
