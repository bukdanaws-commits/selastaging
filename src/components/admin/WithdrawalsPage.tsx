...
import { useWithdrawals, useApproveWithdrawal, useRejectWithdrawal, useUploadTransferProof, useCheckDokuStatus } from '@/hooks/use-api';
...
type WithdrawalStatus = 'pending' | 'approved' | 'processing' | 'transferred' | 'completed' | 'rejected' | 'dispute';
...
interface WithdrawalItem {
  ...
  method: 'AUTO_DOKU' | 'MANUAL';
  dokuDisbursementId?: string;
  dokuReferenceNo?: string;
  dokuStatus?: string;
  requestedAt: string;
  processedAt?: string;
...
function getWdStatusBadge(status: WithdrawalStatus) {
  const config: Record<WithdrawalStatus, { label: string; className: string; icon?: React.ReactNode }> = {
  ...
  return (
    <Badge variant="outline" className={cn('text-xs font-semibold', c.className)}>
      {c.icon}
      {c.label}
    </Badge>
  );
...
function getMethodBadge(method: 'AUTO_DOKU' | 'MANUAL') {
  if (method === 'AUTO_DOKU') {
    return (
      <Badge variant="outline" className="bg-primary/10 text-primary border-primary/30 hover:bg-primary/15 text-[10px] font-semibold gap-1">
  ...
  return (
    <Badge variant="outline" className="bg-violet-500/10 text-violet-400 border-violet-500/30 hover:bg-violet-500/15 text-[10px] font-semibold gap-1">
...
function ApproveMethodDialog({
  wd,
  onApprove,
  isPending,
}: {
  wd: WithdrawalItem;
  onApprove: (id: string, method: 'AUTO_DOKU' | 'MANUAL') => void;
  isPending: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [selectedMethod, setSelectedMethod] = useState<'AUTO_DOKU' | 'MANUAL'>('AUTO_DOKU');

  const handleApprove = () => {
    onApprove(wd.id, selectedMethod);
    setOpen(false);
  };
      ...
      <DialogContent className="bg-card border-input text-foreground max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground text-sm">
            Pilih metode pencairan untuk withdrawal dari <span className="font-semibold text-foreground">{wd.organizerName}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-3">
          ...
          <div className="p-3 rounded-lg bg-background border border-border space-y-1">
            ...
            <p className="text-xs text-muted-foreground mt-1">Bank: {wd.bankName} — {wd.bankHolder}</p>
            <p className="text-[10px] text-muted-foreground font-mono">{wd.bankAccount}</p>
          </div>

          <div>
            <p className="text-sm font-medium text-foreground mb-3">Pilih metode pencairan:</p>
            <div className="grid grid-cols-2 gap-3">
              {/* AUTO_DOKU Option */}
              <button
                type="button"
                onClick={() => setSelectedMethod('AUTO_DOKU')}
                className={cn(
                  'relative p-4 rounded-xl border-2 text-left transition-all duration-200',
                  selectedMethod === 'AUTO_DOKU'
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-bold text-foreground">Auto (DOKU)</span>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">Transfer otomatis via DOKU Disburse</p>
                <Separator className="my-2 bg-border" />
                <div className="space-y-1">
                  <p className="text-[10px] text-muted-foreground">Fee: <span className="text-foreground font-medium">~Rp 6.500</span></p>
                  <p className="text-[10px] text-muted-foreground">Speed: <span className="text-emerald-400 font-medium">5-30 menit</span></p>
                </div>
              </button>
              ...
              >
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">Transfer via mobile banking</p>
                <Separator className="my-2 bg-border" />
                <div className="space-y-1">
                  <p className="text-[10px] text-muted-foreground">Fee: <span className="text-emerald-400 font-medium">Gratis</span></p>
                  <p className="text-[10px] text-muted-foreground">Speed: <span className="text-foreground font-medium">Manual</span></p>
                </div>
              </button>
            </div>
          </div>
...
// ─── CHECK DOKU STATUS BUTTON ───────────────────────────────────────────────

function CheckDokuStatusButton({ wd }: { wd: WithdrawalItem }) {
  const checkDokuStatus = useCheckDokuStatus();

  const handleCheck = () => {
    checkDokuStatus.mutate(wd.id, {
      onSuccess: () => {
        toast.success('Status DOKU diperbarui');
      },
      onError: () => {
        toast.error('Gagal mengecek status DOKU');
      },
    });
  };
  ...
  return (
    <Button
      size="sm"
      className="bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20 h-8 px-3 text-xs font-semibold"
      onClick={handleCheck}
      disabled={checkDokuStatus.isPending}
    >
      {checkDokuStatus.isPending ? (
        <>
...
function ProcessingInfo({ wd }: { wd: WithdrawalItem }) {
  return (
    <div className="space-y-1.5">
      {getWdStatusBadge(wd.status)}
      {wd.dokuReferenceNo && (
        <p className="text-[10px] text-muted-foreground font-mono">
          Ref: {wd.dokuReferenceNo}
        </p>
      )}
      {wd.dokuDisbursementId && (
        <p className="text-[10px] text-muted-foreground font-mono">
          Disb: {wd.dokuDisbursementId}
        </p>
      )}
    </div>
  );
...
export function WithdrawalsPage() {
  ...
  const stats = [
    ...
    { label: 'DOKU Processing', value: processingCount, icon: Loader2, color: 'text-blue-400', bg: 'bg-blue-500/10' },
  ...
  const handleApprove = useCallback((id: string, method: 'AUTO_DOKU' | 'MANUAL') => {
    approveWithdrawal.mutate(
      { withdrawalId: id, method },
      {
        onSuccess: () => toast.success(method === 'AUTO_DOKU' ? 'Withdrawal disetujui — DOKU Disbursement diproses' : 'Withdrawal disetujui — Menunggu transfer manual'),
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className={cn('w-9 h-9 rounded-lg flex items-center justify-center shrink-0', stat.bg)}>
                  <stat.icon className={cn('w-4 h-4', stat.color, stat.label === 'DOKU Processing' ? 'animate-spin' : '')} />
                </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border hover:bg-transparent">
                    ...
                    <TableHead className="text-[11px] text-muted-foreground uppercase tracking-wider font-semibold h-10 px-4">Method</TableHead>
                  ...
                  {paged.length === 0 ? (
                    ...
                    paged.map((wd) => (
                      <TableRow key={wd.id} className="border-border hover:bg-primary/5 transition-colors">
                        ...
                        <TableCell className="px-4 py-3">{getMethodBadge(wd.method)}</TableCell>
                        <TableCell className="px-4 py-3">
                          {wd.status === 'processing' ? (
                            <ProcessingInfo wd={wd} />
                          ) : (
                            getWdStatusBadge(wd.status)
                          )}
                        </TableCell>
                        ...
                        <TableCell className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1">
                            {wd.status === 'pending' && (
                              <>
                                <ApproveMethodDialog wd={wd} onApprove={handleApprove} isPending={approveWithdrawal.isPending} />
                                <RejectModal wd={wd} onReject={handleReject} />
                              </>
                            )}
                            {wd.status === 'processing' && (
                              <CheckDokuStatusButton wd={wd} />
                            )}
                            {wd.status === 'approved' && (
