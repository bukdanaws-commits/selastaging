...
// ProcessPaymentCallback processes a payment callback.
...
func (s *OrderService) ProcessPaymentCallback(orderCode string, paymentType string, transactionStatus string, transactionID string) error {
        var order models.Order
        if err := s.DB.Preload("Items").Preload("Items.TicketType").Preload("Tickets").Preload("Event").Where("order_code = ?", orderCode).First(&order).Error; err != nil {
