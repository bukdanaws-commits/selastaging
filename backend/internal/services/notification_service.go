...
func (s *NotificationService) CreateNotification(userID string, eventType string, title string, message string, data any) error {
        var dataStr string
        if data != nil {
        ...
        // Derive TenantID from user's tenant membership
        var tenantUser models.TenantUser
        tenantID := ""
        if err := s.DB.Where("user_id = ?", userID).First(&tenantUser).Error; err == nil {
...
func (s *NotificationService) CreateNotificationWithCategory(userID string, eventID string, eventType string, category string, title string, message string, data any) error {
        var dataStr string
        if data != nil {
                b, err := json.Marshal(data)
                if err != nil {
