...
type DokuConfig struct {
        ClientID     string
        ClientSecret string // SK-xxx — HMAC-SHA512 signing for API requests
        ...
        PrivateKey   string // RSA private key for B2B token signing (optional)
        IsSandbox    bool
        BaseURL      string
        SnapBaseURL  string
}
...
func Load() {
        ...
        if secretPath := os.Getenv("DOKU_SHARED_KEY_PATH"); secretPath != "" {
                if val := loadSecretFromFile(secretPath); val != "" {
                        Cfg.Doku.SharedKey = val
                }
        }
        if secretPath := os.Getenv("DOKU_BSN_PATH"); secretPath != "" {
                if val := loadSecretFromFile(secretPath); val != "" {
                        Cfg.Doku.BSN = val
                }
        }
